...
const DotField = memo(({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = 'rgba(163, 0, 0, 0.38)',
  ...
  useEffect(() => {
    ...
    function updatePointerPosition(pageX: number, pageY: number) {
    ...
    function onPointerDown(e: PointerEvent) {
      updatePointerPosition(e.pageX, e.pageY);
      mouseRef.current.speed = Math.max(mouseRef.current.speed, 36);
    }
    ...
    function tick() {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if ((p.waveAmplitude as number) > 0) {
          drawY += Math.sin(d.ax * 0.03 + time) * (p.waveAmplitude as number);
          drawX += Math.cos(d.ay * 0.03 + time * 0.7) * (p.waveAmplitude as number) * 0.5;
        }
